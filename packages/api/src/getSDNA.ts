...
export async function getSDNAVersion(
  perspectiveUuid
): Promise<SdnaVersion | null> {
  const ad4mClient = await getAd4mClient();
  const existingSDNALinks = await getSDNALinks(perspectiveUuid);
  if (existingSDNALinks.length > 0) {
    const sdnaLinkVersion = await ad4mClient.perspective.queryLinks(
      perspectiveUuid,
      {
        source: existingSDNALinks[0].data.target,
        predicate: SDNA_VERSION,
      } as LinkQuery
    );
    sdnaLinkVersion.sort((linka, linkb) =>
      linka.timestamp > linkb.timestamp ? -1 : 1
    );
    if (sdnaLinkVersion.length > 0) {
      const sdnaCreatedAt = await ad4mClient.perspective.queryLinks(
        perspectiveUuid,
        {
          source: existingSDNALinks[0].data.target,
      ...
      sdnaCreatedAt.sort((linka, linkb) =>
        linka.timestamp > linkb.timestamp ? -1 : 1
      );
      if (sdnaCreatedAt.length > 0) {
        return {
          version: parseInt(
            sdnaLinkVersion[0].data.target.replace("int://", "")
          ),
...
export async function getSDNALinks(
  perspectiveUuid: string
): Promise<LinkExpression[]> {
  const ad4mClient = await getAd4mClient();
  const existingSDNALinks = await ad4mClient.perspective.queryLinks(
    perspectiveUuid,
    { source: SELF, predicate: ZOME } as LinkQuery
  );
...
export async function getFluxSDNALinks(perspectiveUuid: string) {
  const existingSDNALinks = await getSDNALinks(perspectiveUuid);
  const fluxSDNALinks = existingSDNALinks.filter(
    (link) =>
      link.data.target.includes("flux_message") ||
      link.data.target.includes("flux_post")
  );
  return fluxSDNALinks;
}

export async function getSDNAValues(
  perspectiveUuid: string
): Promise<SDNAValues | null> {
