...
export const fetchData = async (url): Promise<any> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (metas) {
        for (let meta of metas) {
          ...
          const valid = !!Object.keys(metaTags).filter(
            (m: any) => metaTags[m].toLowerCase() === name.toLowerCase()
          ).length;
          // There is no else statement
          /* istanbul ignore else */
          if (valid) {
      ...
      result.video = result[ogVideo]
        ...
        : null;
      if (result.video) {
        result[ogVideoWidth] = result[ogVideoWidth]
          ? result[ogVideoWidth]
          : 560;
