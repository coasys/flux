...
export function useModel<T extends Ad4mModel>(props: Props<T>): Result<T> {
  const { perspective, model, query = {}, preserveReferences = false, pageSize, location } = props;
  ...
  async function subscribeToCollection() {
    try {
      ...
      console.log('useModel (Vue) subscribeToCollection', location, perspective.value.uuid);
