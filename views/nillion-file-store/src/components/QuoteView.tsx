...
interface QuoteViewProps {
  ...
  setVariant: (variant: '' | 'success' | 'danger' | 'warning' | undefined) => void;
  ...
  const handlePayAndRetrieve = async () => {
    try {
      ...
      if (error.message.includes('PermissionError')) {
        setNotificationText('You have not been granted permission to access this file.');
        setVariant('danger');
        setShowNotification(true);
      }
    ...
    <>
      <j-box>
        <j-flex j="center" a="center" wrap gap="500" direction="column">
          <j-text uppercase color="ui-800" size="200" nomargin variant="heading-lg">
            Storage Quote:
          </j-text>
          ...
          {quote.type == 'get' && !showLoader && <j-button onClick={() => handlePayAndRetrieve()}>Download</j-button>}
        </j-flex>
      </j-box>
    </>
  );
