...
export default function VoiceVideo({ webRTC }: Props) {
  ...
  const videoDevices = webRTC?.devices?.filter((d) => d.kind === 'videoinput').filter((d) => !!d.deviceId);
  const audioDevices = webRTC?.devices?.filter((d) => d.kind === 'audioinput').filter((d) => !!d.deviceId);
