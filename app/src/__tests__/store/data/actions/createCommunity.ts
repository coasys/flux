...
import { PERSPECTIVE_DIFF_SYNC } from 'utils/constants/languages';
...
describe('Create Community', () => {
  ...
  beforeEach(() => {
    ...
    // @ts-ignore
    jest.spyOn(ad4mClient.languages, 'byAddress').mockImplementation(async (sourceLanguageHash) => {
    ...
    // @ts-ignore
    jest.spyOn(ad4mClient.neighbourhood, 'publishFromPerspective').mockImplementation(async () => {
      return 'neighbourhood://QmW6LEDuWFuRPBgWsaeh3D9KpjVLu3PZ7VeeV2WYATTJop';
    });

    // @ts-ignore
    jest.spyOn(ad4mClient.languages, 'applyTemplateAndPublish').mockImplementation(async () => {
    ...
    // @ts-ignore
    jest.spyOn(createNeighbourhoodMeta, 'createNeighbourhoodMeta').mockImplementation(async (name, desc, lang) => {
      return [
        {
          ...createChannelMeta,
          hash: () => 12345,
    ...
    // @ts-ignore
    jest.spyOn(ad4mClient.expression, 'create').mockImplementation(async () => {
    ...
    // @ts-ignore
    jest.spyOn(ad4mClient.perspective, 'addLink').mockImplementation(async (perspective, link) => {
  ...
  test('Create Community - Success', async () => {
    ...
    expect(Object.keys(dataStore.neighbourhoods)).toStrictEqual(['9cac577c-0b0a-44f4-9d4f-66edcc236021']);
  });
  ...
  test('Create Community - Failure', async () => {
    ...
    try {
      ...
      expect(error).toHaveProperty('message', 'Error: Could not create Perspective');
    }
