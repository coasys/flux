...
describe('Update Community', () => {
  ...
  beforeEach(() => {
    // @ts-ignore
    jest.spyOn(ad4mClient.perspective, 'addLink').mockImplementation(async () => {
  ...
  test('Update Community - Success', async () => {
    ...
    expect(dataStore.neighbourhoods[community.state.perspectiveUuid].name).toBe('hello');
    expect(dataStore.neighbourhoods[community.state.perspectiveUuid].description).toBe('hello');
  });
});
