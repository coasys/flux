...
export default defineComponent({
  ...
  setup() {
    ...
    async function runActiveAgents() {
    ...
    async function setActiveAgents(diffTime: number) {
      console.log("Delaying setting active agent by", diffTime / 1000);
      setTimeout(() => {
        noDelaySetInterval(async () => {
          runActiveAgents();
        }, agentRefreshDurationMs);
      ...
      async (newValue) => {
        if (newValue.value == true) {
          ...
          if (dif > agentRefreshDurationMs) {
            await setActiveAgents(0);
          } else {
            await setActiveAgents(dif);
          }
