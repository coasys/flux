...
import {
  default as SharedPerspective,
  SharingType,
} from "ad4m/SharedPerspective";
...
export default async function createGroup(name: string): Promise<[string, string]> {
  console.log("Received create", name);
  const perspective = await addPerspective({ variables: { name: name } }).then(
    (result) => {
